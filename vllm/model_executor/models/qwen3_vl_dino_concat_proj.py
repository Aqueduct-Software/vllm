...
class Qwen3_VisionTransformer(nn.Module):
    def __init__(
        ...
        # Early fusion projector for pre-computed DINO embeddings
        DINO_HIDDEN_SIZE = 768
        fused_hidden_size = self.hidden_size + DINO_HIDDEN_SIZE
        self.project_fused = nn.Sequential(
            nn.LayerNorm(fused_hidden_size),
            nn.Linear(fused_hidden_size, self.hidden_size),
        )
    ...
    def forward(
        ...
        *,
        ...
        # ── Early fusion: concat DINO patch tokens + project ──
        if dino_embeds is not None:
            ...
            if dino_aligned.shape[0] == hidden_states.shape[0]:
                hidden_states = self.project_fused(
                    torch.cat([hidden_states, dino_aligned], dim=-1)
                )
        ...
        for modality in mm_input_by_modality:
            ...
            if modality == "image":
                image_embeddings = self._process_image_input(
                    multimodal_input, dino_embeds=dino_embeds
                )
